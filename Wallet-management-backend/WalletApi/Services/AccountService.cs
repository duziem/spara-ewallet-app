...
namespace WalletApi.Services
{
    ...
    using BCrypt = BCrypt.Net.BCrypt; // import Bcrypt module
    ...
    public class AccountService : IAccountService
    {
        ...
        public AuthenticateResponse Authenticate(AuthenticateRequest model, string ipAddress)
        {
            ...
            jwtToken = _jwtUtils.GenerateJwtToken(account);
        ...
        {
            if(_context.Accounts.Any(x => x.VerificationToken == token))
            {
                return;
            }
            else
            {
                throw new AppException("incorrect otp entered");
            }
        }
       ...
       /* private string generateVerificationToken()
        ...
        private void sendVerificationEmail(Account account, string origin)
        {
            ...
            if (!string.IsNullOrEmpty(origin))
            {
                ...
                var verifyUrl = account.VerificationToken;
                message = $@"<p>Please enter the below otp into the site to verify your account:</p>
                            <p>{verifyUrl}</p>";
            }
            else
            {
                ...
                message = $@"<p>Please use the below token to verify your account <code>/accounts/verify-email</code> api route:</p>
